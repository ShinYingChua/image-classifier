...
def classify_images(images_dir, results_dic, model):
    ...
    for filename in results_dic.keys():
        # Call the classifier function to get the classifier label for the image
        classifier_label = classifier(
            images_dir + filename, model).lower().strip()

        # Extract the pet label from the results dictionary
        pet_label = results_dic[filename][0]

        # Check if the pet label is found within the classifier label
        # If so, set the match to 1, otherwise set it to 0
